//[부서정보]
    ...
    let salGrade = [ 
        ...
        // 1. 부서별 평균월급여 
        console.log(`1번) 부서별 평균월급여\n======================`);
        dData.forEach((part)=>{
            let empArr=eData.filter(emp=>emp.dno===part.no);
            let sum=empArr.reduce((sumSalary,emp)=>{
                sumSalary+=parseInt(emp.salary);
                return sumSalary;
            },0);
            let avg=sum/empArr.length;
            if(isNaN(avg)){
                avg=0;}
            console.log(`${part.dname} 평균월급여 : $${avg.toFixed(1)}`)
            
        })

        // 2.부서별 전체 사원수와 커미션을 받는 사원들의 수
        console.log(`\n2번) 부서별 전체사원수 & 커미션 받는 사원수\n==========================================`);
        dData.forEach((part)=>{
            let empArr=eData.filter(emp=>emp.dno===part.no);
            let cntAll=empArr.length;
            let cntComm=empArr.reduce((cnt,emp)=>cnt+(emp.commission!='0'),0);
            console.log(`부서[${part.dname}] => 사원수(${cntAll}명), 커미션 받는 사원수(${cntComm}명)`)
        });
        ...
        dData.forEach((part)=>{
            let empArr=eData.filter(emp=>emp.dno===part.no);
            let map=empArr.map(salary=>)
            {
                    return parseInt(emp.salary);
                }
            });
            console.log(empArr);
            // let max=Math.max(...empArr);
            // let min=Math.min(...empArr);
            // console.log(`부서[${part.dname}] => 최대 급여($${max}), 최소 급여($${min})`)
        });        
